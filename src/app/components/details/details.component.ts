...
export class DetailsComponent implements OnInit {
  ...
  constructor(
    ...
    public http: GetWeatherApiService,
    public mode: ModeToggleService
  ) {}
  ...
  ngOnInit(): void {
    ...
    //  通过传过来的城市 获取该城市的id
    this.http.getCityID(this.city).then((res: any) => {
      // this.cityID = res.location[0].id
      // console.log(res.location[0].id)
      //  通过城市id获取天气数据

      this.http.getWeatherData(res.location[0].id).then((res: any) => {
        // console.log(res)
        this.cityData = res;
      ...
      //  获取五天后的数据
      this.http.getWeatherDataMate(res.location[0].id).then((res: any) => {
        res.daily.forEach((data: any) => {
          if (res.daily.length) {

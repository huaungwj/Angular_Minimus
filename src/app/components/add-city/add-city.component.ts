...
export class AddCityComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    new Promise((resolve, reject) => {
      this.https
        .get(
          'https://geoapi.heweather.net/v2/city/top?key=27760125f4f1448faa7ec3cf16dbce45&number=5&range'
        )
        .subscribe((res) => {
          resolve(res);
        });
  ...
  getCityList() {
    let citys: any = [];
    let input: any = document.querySelector('.search-city-input');
  ...
  showItem(e: any) {
    ...
    //搜索框
    let searchContent: HTMLElement | null =
  ...
  async hiddenItem(e: any) {
    ...
    if (e.keyCode == 13) {
      ...
      //获取输入的值发送给给请求数据
      let cityname: any = await this.http.getCityID(input.value);
      // @ts-ignore
      let data = await this.http.getWeatherData(cityname.location[0].id);
      // @ts-ignore
      let mateData = await this.http.getWeatherDataMate(
        cityname.location[0].id
      );
      // @ts-ignore
  ...
  async keyUpInputVlaue(e: any) {
    ...
    if (e.keyCode == 13) {
      ...
      let mateData: any = await this.http.getWeatherDataMate(
        cityname.location[0].id
      );
      // @ts-ignore
      data.city = cityname.location[0].name;
